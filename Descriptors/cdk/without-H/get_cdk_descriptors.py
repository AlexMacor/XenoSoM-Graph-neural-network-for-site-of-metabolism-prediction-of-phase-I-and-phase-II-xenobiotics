...
def compile_java(cdk_jar_path, java_src_dir):

    cmd = ["javac","-d", java_src_dir, "-classpath", cdk_jar_path,
           os.path.join(java_src_dir, "AtomicDescriptorCalculator.java"),
           os.path.join(java_src_dir, "AtomInfo.java")]


    subprocess.run(cmd, check=True)
    print("compiled")
    
def run_atomic_descriptor_calculator(multi_sdfs, multi_sdfs_out_cdk, cdk_jar, java_src_dir, class_name="AtomicDescriptorCalculator"):
    ...
    for filename in os.listdir(multi_sdfs):
        if filename.endswith(".sdf"):
            ...
            cmd = ["java","-classpath",f"{java_src_dir};{cdk_jar}",class_name,infile,outfile_sdf,outfile_csv]
...
if __name__ == "__main__":
    multi_sdfs = r"path containing all the sdf file" 
    java_src_dir = r"path containing all the necessaries java file"
    cdk_jar_file = r"path cdk, should be the same as java_src_dir\cdk-2.2.jar"
    multi_sdfs_out_cdk = r"path output cdk"
    csv_cdk_out = r'generic path to save csv with missing values\cdk_missing_values.csv'
    
    compile_java(cdk_jar_file, java_src_dir)
    run_atomic_descriptor_calculator(multi_sdfs, multi_sdfs_out_cdk, cdk_jar_file, java_src_dir)
