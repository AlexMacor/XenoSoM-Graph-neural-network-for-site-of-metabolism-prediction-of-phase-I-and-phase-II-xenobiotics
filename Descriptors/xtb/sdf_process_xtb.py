...
# keep this commented  #opt_sdf_files_xtb_path = "/mnt/c/Users/Alessio Macorano/Desktop/Database-test-reazione-equilibrioum/opt/xtb/OPT"
    ...
    parser = argparse.ArgumentParser(description="Conformational + XTB pipeline")

    parser.add_argument("--multi_sdfs", required=True,
                        help="Folder containing initial sdf molecules")

    parser.add_argument("--input_cdk_xtb", required=True,
                        help="Folder where conformers will be saved")

    parser.add_argument("--charges_csv", required=True,
                        help="Path to output CSV containing charges")

    parser.add_argument("--in_path", required=True,
                        help="Folder where to copy conformers before XTB run optimization (step 1)")

    parser.add_argument("--temp_path", required=True,
                        help="Temporary folder used by XTB optimization (step 2)")

    parser.add_argument("--fin_path", required=True,
                        help="Folder where optimized structures will be copied (step 3)")

    parser.add_argument("--sh_file_path", required=True,
                        help="Bash script path for XTB optimization (charge assignment)")

    parser.add_argument("--SP_bash_script", required=True,
                        help="Bash script path for XTB single-point calculation")

    parser.add_argument("--num_of_conformer", type=int, default=10,
                        help="Number of conformers to generate (default = 10)")

    args = parser.parse_args()

    multi_sdfs = args.multi_sdfs
    input_cdk_xtb = args.input_cdk_xtb
    charges_csv = args.charges_csv
    in_path = args.in_path
    temp_path = args.temp_path
    fin_path = args.fin_path
    sh_file_path = args.sh_file_path
    SP_bash_Script = args.SP_bash_script
    num_of_conformer = args.num_of_conformer
    
    conformational_analysis(multi_sdfs, input_cdk_xtb, num_of_conformer=num_of_conformer)
    ...
    try:
        optimization_xtb = subprocess.run(
            ["python", "Run_xtb_conv.py", "--n_folders", str(num_files)],
            capture_output=True, text=True, check=True
        )
        print("Output:", optimization_xtb.stdout)
    except subprocess.CalledProcessError as e:
        print("Errore subprocess:")
