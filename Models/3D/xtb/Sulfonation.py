...
for subclass in subclass_folders:
    ...
    xtb_desc_list = get_xtb_descriptors(path, path_csv_descr_subclass)
    data_list = []
    
    for mol, nome_molecola, features in zip(mol_rdkit, nomi_all_dataset, xtb_desc_list):
    ...
    train_features = [data.xtb_desc for data in train_graph] 
    train_features_all = torch.cat(train_features, dim=0) 
    
    scaler.fit(train_features_all.numpy()) 
    
    for data in train_graph:
        data.xtb_desc = torch.from_numpy(scaler.transform(data.xtb_desc.numpy())).float()
    ...
    save_dir = os.path.join(root_path, 'Risultati-sottoclassi', subclass, 'xtb')
