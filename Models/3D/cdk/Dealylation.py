...
parser = argparse.ArgumentParser(description="Training with cdk descriptors")
parser.add_argument('--root_path', type=str, required=True, help="main path (es. '.../Dealkylation')")
parser.add_argument('--descr_path', type=str, required=True, help="Path of cdk descriptors")
args = parser.parse_args()

root_path, path_csv_descr_subclass = args.root_path, args.descr_path
use_cdk = True
...
for subclass in subclass_folders:
    ...
    for mol, nome_molecola, features in zip(mol_rdkit, nomi_all_dataset, cdk_desc_list):
