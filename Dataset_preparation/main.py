...
# CONTROLLARE SE IN QUESTO SCRIPT HO FILE INTERMEDI DA ELIMINARE
...
# match cartella csv nomi 
def controlla_match_cartelle_csv(base_path):
    ...
    for cartella in cartelle:
        if cartella in file_csv:
            ...
            print(f"Match: cartella e file CSV → {cartella}")
        else:
            no_match.append(cartella)
            print(f"Nessun CSV per la cartella → {cartella}")
                ...
                if 'Name' in df.columns:
                    ...
                    for name in names:
                        ...
                        if os.path.exists(source_path):
                            ...
                            print(f"NON TROVATO: {sdf_filename}")
            else:
                print(f"Nessun match cartella ↔️ CSV: {csv_name}")
    ...
    for dirpath, dirnames, filenames in os.walk(base_path):
        for filename in filenames:
            if filename.endswith('.csv') and not filename.endswith('_indice_atomi_da_sdf.csv'):
                ...
                # file csv per le classi devono essere rimossi e 
                # rigenerati con la funzione successiva
                ...
                if base_name in name_to_rmv and os.path.exists(output_path):
                    ...
                    print(f"Eliminato il file per le classi: {output_path}")
...
if __name__ == "__main__":
    ...
    for sub in os.listdir(base_path):
        ...
        if os.path.isdir(subfolder_path):
            match, no_match = controlla_match_cartelle_csv(subfolder_path)
